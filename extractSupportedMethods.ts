// This script extracts the supported methods for each provider
...
for (const provider in Providers) {
  ...
  for (const method in providerConfig) {
    ...
    if (method === 'chatComplete') {
      if (providerConfig[method].tools)
        output[provider].push(supportedMethods.toolCalling);
    }
  }
}
...
fs.writeFileSync(
  'output/providerSupportedMethods.json',
  JSON.stringify(output, null, 2),
  'utf8'
);
console.log('Output written to output/providerSupportedMethods.json');
